...
using ConsoleApp3.Project.TelegramBot.Scenarios;
using Telegram.Bot;
...
class Program
{
    static async Task Main()
    {
        ...
        var connectionString = "Host=localhost;Port=5432;Database=ToDoList;Username=postgres;Password=your_password";

        // Создание фабрики DataContext
        var dataContextFactory = new DataContextFactory(connectionString);

        // SQL репозитории
        IUserRepository userRepo = new SqlUserRepository(dataContextFactory);
        IToDoRepository todoRepo = new SqlToDoRepository(dataContextFactory);
        IToDoListRepository todoListRepo = new SqlToDoListRepository(dataContextFactory);
        ...
        try
        {
            ...
            Console.WriteLine($"Подключен к базе данных: {connectionString.Split(';')[2]}");
