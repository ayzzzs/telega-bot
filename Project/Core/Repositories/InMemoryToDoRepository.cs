...
namespace bbbbb.Project.Core.Repositories
{
    public class InMemoryToDoRepository : IToDoRepository
    {
        ...
        {
            var list = _tasks.Where(x => x.User.UserId == userId && x.State == ToDoState.Active).ToList();
            return Task.FromResult((IReadOnlyList<ToDoItem>)list);
        }
        ...
        {
            var exists = _tasks.Any(x => x.User.UserId == userId && string.Equals(x.Name, name, StringComparison.OrdinalIgnoreCase) && x.State == ToDoState.Active);
            return Task.FromResult(exists);
        }

        public Task<int> CountActiveAsync(Guid userId, CancellationToken cancellationToken)
        {
            var count = _tasks.Count(x => x.User.UserId == userId && x.State == ToDoState.Active);
